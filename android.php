<?php
	...
	while (1 == 1) {
		...
		if ($player->ronda == 1 || $player->espera == 'off' ) {
			...
			if ( $info->nxround_lnumber == '1' || $info->nxround_lnumber == '3' || $info->nxround_lnumber == '4' || $info->nxround_lnumber == '7' || $info->nxround_lnumber == '8' || $info->nxround_lnumber == '9') {
		...
		if ($player->lhash != $info->lround_hash && $player->accion == 'on') {
			...
			if ($player->posicion_value < 2) {
				...
				$player->posicion = $player->posicion * 2;
            ...
            //si perdemos 7
            if ($player->perdidas == $player->perdida_maxima || $player->ganancias >= $params->salida) {
                
    
                //calculamos espera
                $ahora = strtotime("now");
                $mañana = strtotime("tomorrow 00:00:00");
                $espera = $mañana - $ahora;

                echo "\r\n  Se ha alcanzado un maximo. Esperamos desde ahora ($ahora) a manana ($mañana): $espera segundos. \r\n";
