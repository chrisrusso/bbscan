...
model.add(LSTM(32, input_shape=(1, lookback), return_sequences=True))
...
while True:
    ...
    if  ((informacion_xnew != replica_xnew).any() or replica_xnew is None) :
        ...
        #la enviamos a la db
        cursor.execute("""INSERT INTO prediccionador_6 (y_previa, diferencia_previa, prediccion_previa, valor, clase, x, y, diferencia, prediccion) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(y_previa, diferencia_previa, prediccion_previa, valor, clase, cadena_x, cadena_y, diferencia, prediccion))
        conn.commit()

        imprimir("\n")
        imprimir("prediccion_previa = %s, valor = %s, y = %s, diferencia = %s, prediccion = %s" % (prediccion_previa, valor, cadena_y, diferencia, prediccion))
