...
# se usan los mecanismos del prediccionador 5 buscando mayor performance.
...
archivo                 = read_sql_query("SELECT value FROM ( SELECT * FROM crawler where round > %d order by round ASC LIMIT 0, %d) sub ORDER BY round ASC" % (comienzo, sample), conn)
...
model = load_model('modelos/model_6.h5')


#limpiamos la base
cursor.execute("""TRUNCATE TABLE prediccionador_7""")
conn.commit()
...
cadena_y                    = 0
pruebas                     = 20000
round                       = comienzo + sample + 1
maximo                      = round + pruebas
cadena_diferencia           = None
cadena_diferencia_previa    = 0

while round < maximo:

    #pedimos la informacion
    archivo_xnew         = read_sql_query("select value from (select * from crawler where round <= %d order by round desc LIMIT 0,%d) sub order by round asc" % (round, lookback), conn)
    ...
    cadena_previa_ynew          = cadena_y
    cadena_diferencia_previa    = cadena_diferencia
    ...
    if  ((informacion_xnew != replica_xnew).any() or replica_xnew is None) :
        ...
        cadena_diferencia = "".join(map(str, diferencia))
        cadena_diferencia = cadena_diferencia.strip("[]")
        
        #seleccionamos el valor para comodidad de análisis
        cursor.execute("""SELECT value FROM crawler WHERE round = %d""" % round )
        ...
        #la enviamos a la db
        cursor.execute("""INSERT INTO prediccionador_7 (previa_y, previa_diferencia, previa_prediccion, valor, class, x, y, diferencia, prediccion) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",(cadena_previa_ynew, cadena_diferencia_previa, prediccion_previa,  valor, clase, cadena_x, cadena_y, cadena_diferencia, prediccion))
        conn.commit()

        imprimir("\n")
        imprimir("prediccion_previa = %s, valor = %s, x = %s, y = %s, diferencia = %s, prediccion = %s" % (cadena_diferencia_previa, valor, cadena_x, cadena_y, cadena_diferencia, prediccion))
